...
from airfoil_lny.core.reporting.utils import (
    read_yaml,
    ensure_dir,
    timestamp_slug,
    write_json,
...
from airfoil_lny.core.optimization.optimizers.random_search import (
...
from airfoil_lny.core.export.drawing_2d import (
    airfoil_closed_polyline,
    export_svg,
    export_dxf_polyline,
)
...
def _make_run_dir(cfg: dict) -> Path:
    """
    Creates the run directory under project.out_dir.

    If out_dir is relative, it is interpreted relative to the repo root (airfoil-lny/).
...
def _write_dat(dat_path: Path, px, py, title: str) -> None:
...
def run_study(config_path: Path) -> Path:
    ...
    for it in range(n_iters):
        ...
        for cand_i, cand in enumerate(candidates):
            # 1) geometry: NACA4(m,p,t)
            _, xu, yu, xl, yl = naca4_coords(
                cand["m"],
                cand["p"],
                cand["t"],
                n_points,
            ...
            # 2) constraints penalty (thickness bounds)
            cres = evaluate_constraints(cand, constraints_cfg)

            # 3) XFOIL evaluation near target AoA
            ...
            # small sweep around target AoA for stability
            a0 = aoa0 - 0.5
            a1 = aoa0 + 0.5
            astep = 0.5
        ...
        # Log per-iteration best
        if best_local is not None:
            tracker.log(best_local)

    # -------------------------
    # Postprocess exports for GLOBAL BEST
    # -------------------------
    best = tracker.best
    if best is not None:
        ...
        # full polar sweep for plots on best
        ...
        rows = parse_xfoil_polar(polar_path) if (code == 0 and polar_path.exists()) else []
        if rows:
        ...
        write_json(
            run_dir / "study_summary.json",
            {
                ...
                "best_metrics": {"CL": best.L, "CD": best.D, "LD": (best.L / best.D) if best.D > 0 else None},
                "exports": {
                    "dir": str(export_dir),
